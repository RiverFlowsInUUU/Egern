...
```
egern-anti-dns-leak/
├── profiles/            # 12 份配置 = 6 个版本 × 2 种形态（详见下方「六个版本」）
...
- **`v2.3` = `v2.2` + 4 处修复**（`MAX` 的 `filter`、`Korea` 的冗余上游、`Smart` 的 `flatten`、`ChatGPT` / `Gemini` 的空组）—— 只动 `policy_groups` 段、5 个组，`dns` 段与规则逐行相同。对照表见 [`docs/04` §4](docs/04-模板逐段讲解.md#4-policy_groups-段)。
- **`v2.2` = `v2.1` 精简机场槽位**（4 → 2）—— 同样只动 `policy_groups` 段，`dns` 段与规则逐行相同。
- **`v2.1` 与 `v2` 逻辑逐项等价** —— 两者只差 52 行「值恰好等于官方默认值」的冗余行（`ipv6: false` / `flatten: false` 这类，写与不写是同一份配置），各审计脚本输出完全一致。
- **`v1` 与 `v2.1` 的差别只在 `dns` 段**（40 行 vs 22 行），其余段逐行相同。防泄露能力经审计脚本实测**逐项等价**。
- **`v0` 是极简裁剪** —— 只留 `Proxy` / `AI` / `AD` / `Final` 4 个组、9 条规则。
- ⚠️ 别把文件名 `v0` / `v1` / `v2` / `v2.1` / `v2.2` / `v2.3` 与 [`docs/06`](docs/06-实测数据与版本谱系.md) 里的 **v1~v8 迭代谱系**（v9–v10 见 [`DetailsReadme/`](DetailsReadme/) §3）混淆 —— 前者是**文件版本**，后者是配置**自身的历史迭代**，两个维度。
...
**回归测试**：`bash skill/tests/run.sh` —— 两阶段共 22 个断言（阶段 1：5 个 fixture × 2 个脚本 = 10；阶段 2：12 份 profile 的地区 filter 同步 = 12），退出码非 0 即失败。
...
- ✨ **`v2.3`（当前推荐版）** —— `v2.2` 的修正后继，修 4 处（`MAX` 的筛选规则、`Korea` 的冗余上游、`Smart` 的 `flatten`、`ChatGPT` / `Gemini` 的空组）；`dns` 段与规则逐行未变。**导入前只剩订阅地址要填**，明细见 [`docs/04` §4](docs/04-模板逐段讲解.md#4-policy_groups-段)。

**变更**

- 🔧 **推荐版当天两次移交** —— `v2.1` → `v2.2` → `v2.3`；`v2.2` 降为保留版。
