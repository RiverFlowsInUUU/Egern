...
| 版本 | 相对前一版的变化 |
|:----:|:-----|
| `v1` | 起点：最老的完整版（`dns` 段 40 行） |
| `v2` | `dns` 段精简 40 → 22 行；防泄露能力经审计脚本实测逐项等价 |
| `v2.1` | 删掉 52 行「值恰好等于官方默认值」的冗余行（`ipv6: false` / `flatten: false` 这类，写与不写是同一份配置） |
| `v2.2` | 机场槽位 4 → 2 |
| `v2.3` | 修 4 处：`MAX` 的 `filter`、`Korea` 的冗余上游、`Smart` 的 `flatten`、`ChatGPT` / `Gemini` 的空组 —— 明细见 [`docs/04` §4](docs/04-模板逐段讲解.md#4-policy_groups-段) |

> ⚠️ 旧版**全部缺 `update_interval`** ⇒ 规则集可能长期停在首次下载的版本，新收录的域名会一直命不中 —— 这是 `v2.4` 修掉的最实质问题。
>
