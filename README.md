...
```
egern-anti-dns-leak/
├── profiles/            # 14 份配置 = 7 个版本 × 2 种形态（详见下方「七个版本」）
...
- **`v2.4` = `v2.3` + 给全部 21 条 `rule_set` 补 `update_interval: 86400`** —— 规则集按天自动刷新，不再停在首次下载的版本。`dns` 段、规则内容与顺序、`policy_groups` 段**逐字未变**。旧版缺此字段，规则集可能长期不更新（新收录的域名会一直命不中）。
...
- ⚠️ 别把文件名 `v0` / `v1` / `v2` / `v2.1` / `v2.2` / `v2.3` / `v2.4` 与 [`docs/06`](docs/06-实测数据与版本谱系.md) 里的 **v1~v8 迭代谱系**（v9–v10 见 [`DetailsReadme/`](DetailsReadme/) §3）混淆 —— 前者是**文件版本**，后者是配置**自身的历史迭代**，两个维度。
...
> 记录**模板本身**的显著变动，按时间倒序。配置文件版本的说明见上方 [📦 七个版本](#-七个版本)。
