...
```
1. 挑一份配置   →  推荐 profiles/v2.3.yaml
2. 填订阅       →  Airport-A / Airport-B 两处 sub.example.com
3. 填节点       →  proxies 段（模板为空 []，只用订阅可跳过）
4. 导入 Egern   →  完成
```
...
| 位置 | 现状 | 填什么 |
|:----:|:----:|:------:|
| `Airport-A` / `Airport-B` 的 `urls` | `sub.example.com` 占位 | 你的订阅地址（**只有这两处**） |
| `proxies` | `[]` | 你的自建节点（**只用机场订阅的话可以留空**） |

> `v2.3` 起**所有分流组的 `policies` 都已填好**（`ChatGPT` / `Gemini` 也补上了 `Proxy`），
> 不再需要你逐个补空组 —— 填完订阅即可直接导入。

订阅方式：把 `Airport-A` / `Airport-B` 里的 `sub.example.com?token=REPLACE_WITH_YOUR_TOKEN` 换成你自己的订阅地址。
...
├── profiles/
│   ├── v2.3.yaml        # 推荐 · 带注释
│   ├── v2.3.min.yaml    # 推荐 · 纯配置
│   ├── v2.2.yaml        # 保留 · 带注释（与 v2.3 差 4 处修正）
│   ├── v2.2.min.yaml    # 保留 · 纯配置
...
| 版本 | 策略组 | 规则 | 机场槽位 | 定位 |
|:----:|:------:|:----:|:--------:|:-----|
| **`v2.3`** ⭐ | 27 | 24 | **2** | **推荐** · `v2.2` 的修正后继（4 处修复，见下） |
| `v2.2` | 27 | 24 | 2 | 保留 · `v2.1` 的机场槽位精简版 |
...
- **`v2.3` 与 `v2.2` 的差别只在 `policy_groups` 段**，且只有 5 个组（4 处修复）：`MAX` 的 `filter` 改为「收录所有倍率 < 1 的节点」（原写法既误收 `10.1` 这类、又漏收 `0.5` 这类）、`Korea` 的上游去掉冗余的 `Smart`、`Smart` 补上 `flatten: true`、`ChatGPT` / `Gemini` 从空组改为 `[Proxy]` + `flatten: true`。`dns` 段与规则**逐行相同**。
...
以 **`v2.3`** 为例（27 个组 / 24 条规则）。**组与组可以互相引用**，最终都收敛到 `Proxy` 或 `DIRECT`。
...
| 策略组 | 类型 | 说明 |
|:------:|:----:|:-----|
| `Airport-A` | `external` | 订阅槽位 ①（模板是 `sub.example.com` 占位，**必须换成你的**） |
| `Airport-B` | `external` | 订阅槽位 ②（同上，另带一条 `urls_disabled` 示例） |

> `v2.1` 及更早的版本有 **4 个**槽位（多出 `Airport-C` / `Airport-Free`）。`v2.2` 把 `C` 与 `Free` 删掉：它们的引用面只有 `Smart`（和 `Japan`），上游并入 `Airport-A` / `Airport-B` 后**不损失任何选路能力**，但少填两个订阅地址。
...
| `ChatGPT` | `fallback` | 故障转移 · 子策略 `Proxy` + `flatten: true`（在 `Proxy` 展开后的**全部节点**里按顺序取第一个可用的） |
| `Gemini` | `fallback` | 同上（`Google` 组的**首项就是它**，默认选中） |
| `Claude` | `smart` | 默认指向 `Taiwan` |

> 🔧 `ChatGPT` / `Gemini` 在 `v2.2` 及更早版本里是**空组**（`policies: []`），而规则 4/5 **直接指向它们**
> （`OpenAI.list → ChatGPT`、`Gemini.list → Gemini`）⇒ 导入即**静默断流**。`v2.3` 填入 `Proxy` 并加 `flatten: true`：
> 不加 `flatten` 时 `fallback` 只有「`Proxy`」**一个候选**，等于没有故障转移能力；加上之后才真正展开成节点级。
> （官方「通用字段」明确 `flatten` 在 `select` / `auto_test` / `smart` / `fallback` / `load_balance` 五种基础类型上通用。）
...
| 🔗 **节点可选** | `proxies` 是空 `[]`；只用机场订阅的话可以留空，有自建节点才需要填 |
...
- ✨ **`v2.3`（推荐版）** —— `v2.2` 的修正后继，**四处修复**（`dns` 段与规则逐行未变）：
  ① `MAX` 的 `filter` 换成 `(?<![\d.])0\.\d*[1-9]`，收录**所有倍率 < 1** 的节点（原写法只认字面 `0.01` / `0.1`，既误收 `10.1 倍率`、又漏收 `0.5` / `0.99`）；
  ② `Korea` 的上游去掉冗余的 `Smart`（`v2.2` 起 `Smart ⊂ (Airport-A ∪ Airport-B)`，候选被重复展开）；
  ③ `Smart` 补上 `flatten: true`，候选从「A 组 / B 组」两个单位变成**全部具体节点**；
  ④ `ChatGPT` / `Gemini` 从**空组**改为 `[Proxy]` + `flatten: true` —— 原先是 `policies: []` 而规则直接指向它们，**导入即静默断流**。
- ✨ **`v2.2`** —— 机场订阅槽位 **4 → 2**（删 `Airport-C` / `Airport-Free`），`MAX` 组改为「带节点筛选的 `Smart`」、不再自带订阅 URL；`dns` 段与规则逐行未变。
- ✨ **`v2.1`** —— `v2` 的精简版：删掉 52 行与官方默认值重复的配置项，**行为完全一致**。

**变更**

- 🔧 **推荐版由 `v2.2` 移交给 `v2.3`** —— `v2.2` 降为保留版，文件本身未动。
- 🎯 **`v2.3` 的必填项从三处降到一处** —— 所有分流组的 `policies` 都已填好，不再需要手动补空组；**只需填 `Airport-A` / `Airport-B` 的订阅地址**（`proxies` 留空也能跑）。
- 🔧 **推荐版由 `v2.1` 移交给 `v2.2`** —— `v2.1` 降为保留版，文件本身未动。
