...
- **`v2.3` = `v2.2` + 4 处修复**（`MAX` 的 `filter`、`Korea` 的冗余上游、`Smart` 的 `flatten`、`ChatGPT` / `Gemini` 的空组）—— 只动 `policy_groups` 段、5 个组，`dns` 段与规则逐行相同。明细见 [更新日志](#-更新日志)。
...
- ✨ **`v2.3`（推荐版）** —— `v2.2` 的修正后继，**四处修复**（`dns` 段与规则逐行未变）：
  ...
  ③ `Smart` 补上 `flatten: true`（候选从两个组单位变成全部具体节点）；
...
- 🎯 **`v2.3` 的必填项从三处降到一处** —— 所有分流组的 `policies` 均已预填完毕。
