...
```
1. 挑一份配置   →  推荐 profiles/v2.2.yaml
2. 填节点       →  proxies 段（模板为空 []）
3. 填订阅       →  Airport-A / Airport-B 两处 sub.example.com
4. 补分流组     →  把空的 policies: [] 填上节点名 / 订阅组名
5. 导入 Egern   →  完成
```
...
以 **`v2.2`** 为例（27 个组 / 24 条规则）。**组与组可以互相引用**，最终都收敛到 `Proxy` 或 `DIRECT`。
