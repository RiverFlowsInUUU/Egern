...
profiles/routing_v2.4.yaml / .min.yaml      # 分流版 · 推荐（脱敏模板：无节点、无订阅、无证书；机场槽位 2 个）
...
docs/07-文件版本沿革.md                     # 两条线 + 分流版 routing_v1→v2.4 逐个说明（含四次改名记录）
...
> **可选版本只有两个** —— `routing_v2.4`（分流版 · 推荐）与 `lazy`（懒人版）；
...
> 换句话说"不要手改仓库里的 yaml"这条老规矩**已作废**：现在的 `routing_v2.1` / `routing_v2.2` / `routing_v2.3` / `routing_v2.4`
...
该放哪：**改动记录 → [`CHANGELOG.md`](../../CHANGELOG.md)；判据与原理 → [`DetailsReadme/`](../../DetailsReadme/DetailsReadme.md) 或 [`docs/`](../../docs/)。**
...
[`docs/11-规则集与来源.md`](../../docs/11-规则集与来源.md) 承接全部规则集信息，
首页只在文件结构 / 更多文档里留一个链接。
