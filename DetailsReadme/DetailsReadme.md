...
- **四个版本 × 双形态**：`profiles/v2.1.yaml`（**推荐**，带注释，每段附原理）+ `profiles/v2.1.min.yaml`（纯配置，结构逐项一致，仅去注释）；`profiles/v2.yaml` / `v2.min.yaml` 原样保留以备对照（与 v2.1 只差 52 行「值恰好等于官方默认值」的冗余行，写与不写是同一份配置）；`profiles/v1.yaml` / `profiles/v1.min.yaml` 为旧版，保留不删；`profiles/v0.yaml` / `v0.min.yaml` 为**极简裁剪版**，只留 `Proxy` / `AI` / `AD` / `Final` 四个组与 9 条规则（v1/v2/v2.1 为 29 组 / 24 条），`dns` 段与 v2.1 逐字相同。**v1 与 v2.1 的差别只在 `dns` 段**，其余顶层段逐字相同——v2.1 把该段从 40 行压到 22 行（删掉无引用点的 `hosts:` 段、与 catch-all 重叠的第二条兜底、并把 6 端点收敛为 4），防泄露能力经审计脚本实测逐项等价。v0 另有两处**只属于 v0** 的调整：`Final` 组加 `hidden: true`（它只有一个子策略，没有手动切换的意义）、`AD` 组去掉 `DIRECT` 子策略只留 `REJECT`（想临时放行就在 `rules` 里加一条更靠前的 `domain` 规则指向 `DIRECT`），**不同步到其他版本**。⚠️ 文件名里的 v0/v1/v2/v2.1 是**文件版本**，与 `docs/06` 的「配置迭代谱系 v1~v10」是两个维度。
